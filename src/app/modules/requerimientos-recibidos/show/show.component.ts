...
export class ShowComponent implements OnInit {
  ...
  constructor(private tableSvc : TableService, private fb: NonNullableFormBuilder, private api: ApiService, private modal: NzModalService, private router: Router, private route: ActivatedRoute) {
  ...
  ngOnInit(): void {
    ...
    this.route.paramMap.subscribe(params => {
      this.id_requerimiento = params.get('id_requerimiento')!;
      // Ahora puedes usar yourParam en tu componente
      console.log(this.id_requerimiento);
      this.buscarRequerimiento();
    });
  }
  ...
  buscarRequerimiento(){
    this.api.consulta('busca-requerimiento/'+this.id_requerimiento, 'get').subscribe((resp) => {
      console.log(resp);


      this.validateFormRequerimiento.controls.id_empresa.setValue(resp.data.id_empresa);
      this.validateFormRequerimiento.controls.id_sucursal.setValue(resp.data.id_sucursal);
      this.validateFormRequerimiento.controls.id_empleado.setValue(resp.data.id_empleado);
      this.validateFormRequerimiento.controls.id_empleado_aprobador.setValue(resp.data.id_empleado_aprobador);
      this.validateFormRequerimiento.controls.nro_requerimiento.setValue(resp.data.nro_requerimiento);
      this.validateFormRequerimiento.controls.fecha_emision.setValue(resp.data.fecha_emision);
      this.validateFormRequerimiento.controls.fecha_ceracion.setValue(resp.data.fecha_creacion);

      resp.data.requerimientos_detalle.forEach(element => {
        this.displayData.push({
